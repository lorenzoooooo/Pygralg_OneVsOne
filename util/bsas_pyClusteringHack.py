...
from numpy import array, sum, argmin, reshape
...
class bsas:
    ...
    "Medoids = false -> hack luca"
    def __init__(self, data, maximum_clusters, threshold, ccore=True, medoids=False, **kwargs):
        """!
        ...
        # Luca Hack
        self._medoidsFlag = medoids
        if (self._ccore and self._medoidsFlag) is True:
            print("Warning! Medoid-like update not implemented with c core \n Using centroid-like")
    ...
    def _update_representative(self, index_cluster, point):
        ...
        # Medoid hack
        if self._medoidsFlag is True:
            ## LUCA
            # transformed_data = reshape( array(self._data[self._clusters[index_cluster]]), (-1,1))#.reshape(-1,1)
            transformed_data = array(self._data[i] for i in self._clusters[index_cluster])                              # .reshape(-1,1)
            if transformed_data.ndim != 2:
                transformed_data = reshape(transformed_data, (-1, 1))
            # get a non condensated form
            distance_matrix = squareform(pdist(transformed_data, lambda x, y: self._metric(x, y)))                      # x[0],y[0]
            # distance_matrix = cdist(transformed_data, lambda x, y: self._metric(x, y))

            s = sum(distance_matrix, axis=0)
            minSOD_item = argmin(s, axis=0)
            rep = self._data[self._clusters[index_cluster][minSOD_item]]
            # print(index_cluster, minSOD_item)
            # ## ALESSIO
            # transformed_data = numpy.array([self._data[i] for i in self._clusters[index_cluster]]).reshape(-1, 1)
            # D = cdist(transformed_data, transformed_data, lambda x, y: self._metric(x[0], y[0]))
            # D = 0.5 * (D + D.T)
            # SoD = numpy.sum(D, axis=0)
            # minSOD_item = numpy.argmin(SoD, axis=0)
            # rep = self._data[self._clusters[index_cluster][minSOD_item]]
        else:
